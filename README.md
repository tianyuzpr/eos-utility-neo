# 📷 EOS Utility Neo

语言 | Language

中文(简体) | [English](./README.EN.md)

## 目录

- [项目描述](#项目描述)
- [现状](#现状)
- [安装说明](#安装说明)
- [使用方法](#使用方法)
- [技术栈](#技术栈)
- [许可证](#许可证)
- [贡献指南](#贡献指南)
- [联系我们](#联系我们)

## 项目描述

本项目基于佳能 EDSDK 开发，旨在为老款 EOS 相机提供现代化的遥控拍摄体验。

原版 EOS Utility 2.x 界面陈旧，对新系统兼容性较差；而 3.x 版本已不再支持老款机型（如 EOS 5D Mark II）。本项目正是为此而生——基于 WebView 构建全新 UI，同时适配现代操作系统，让经典相机焕发新生。

## 现状

🚧 开发中...欢迎您的加入！

加入开发：[联系我们](#联系我们)

## 安装说明

占位符

## 使用方法

占位符

## 技术栈

- [Python 3.13](https://www.python.org/downloads/release/python-31315/)
- [pyedsdk](https://github.com/fmargall/pyedsdk)
- [Flask](https://flask.palletsprojects.com/)
- [pywebview](https://pywebview.flowrl.com/)
- Canon EDSDK（见下方部分地区佳能 EDSDK 官方入口）

## 许可证

- [Apache License 2.0](./LICENSE)
- [Apache License 2.0 中文参考](./LICENSE.ZHCN)
- Canon EDSDK 许可协议请参见佳能官网

<details>
<summary>部分地区佳能 EDSDK 官方入口（点击展开）</summary>

- [佳能中国](https://www.canon.com.cn/supports/sdk/icp/)
- [佳能欧洲开发者项目](https://developers.canon-europe.com/developers)
- [佳能美国开发者支持网站](https://developersupport.canon.com/)
- [佳能亚洲开发者资源](https://asia.canon/en/campaign/developerresources/cap)
- [佳能南非 SDK 页面](https://www.canon.co.za/business/imaging-solutions/sdk/)

</details>

## 贡献指南

参见 [CONTRIBUTING.md](./CONTRIBUTING.md)

## 联系我们

- [Email: xiayue1052@outlook.com](mailto:xiayue1052@outlook.com)
- [GitHub](https://github.com/tianyuzpr)
- [QQ群](https://qm.qq.com/q/Cdzhf9g6Qw)
